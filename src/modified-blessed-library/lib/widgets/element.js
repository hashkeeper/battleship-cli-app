...
Element.prototype.parseContent = function(noTags) {
  ...
  if (this._clines == null
    ...
    if (this.screen.fullUnicode) {
      ...
      content = content.replace(unicode.chars.tall, '$1\x03');
      ...
      content = content.replace(unicode.chars.tall, '??');
...
Element.prototype._wrapContent = function(content, width) {
  ...
  lines.forEach(cur => {
    let rowEmojiCount = 0;
    cur.split(" ").forEach(cell => {
      if(cell.match(unicode.chars.surrogate)) ++rowEmojiCount;
    });
    if(rowEmojiCount > self.emojiCount) self.emojiCount = rowEmojiCount; 
  });
